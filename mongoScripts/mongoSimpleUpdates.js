use nw201501;
db.hwdata.update({"type":"gamer"},{$inc:{"vitals.strength":20}});

db.hwdata.update({"type":"gamer"},{$push:{guilds:"sociedad de almas "}});

db.hwdata.update({"type":"gamer"},{$set:{"vitals.revival":200}});

db.hwdata.update({"type":"gamer"},{$currentDate:{lastlogon:true}});

db.hwdata.update({"type":"gamer"},{$inc:{"items.0.magic":20 ,"gadgets.1.defensepower":20}});
